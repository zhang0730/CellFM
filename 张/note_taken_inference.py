...
#该模块是GEARS系统的评估核心，提供了全面的模型性能评估和分析功能。

def evaluate(loader, model, uncertainty, device, adata): #模型推理主函数 输入：数据加载器、模型、设备等
    """
    ...
    #输出：包含预测结果、真实值和差异表达基因结果的字典
    if uncertainty: #支持不确定性模式
        results['logvar'] = torch.stack(logvar).detach().cpu().numpy()
    
    return results


def compute_metrics(results, gene_idx=None): #计算核心指标
    """
    ...
    #评估指标系统
    metric2fct = {
           'mse': mse, #MSE：量化预测值与真实值的绝对差异
           'pearson': pearsonr #Pearson：衡量预测趋势的一致性
    }
...
def compute_synergy_loss(results, mean_control, high_umi_idx, subtype = 'POTENTIATION'): #基因相互作用分析
...
def non_zero_analysis(adata, test_res): #非零表达基因分析
    metric2fct = {
           'pearson': pearsonr,
           'mse': mse
    }
...
def non_dropout_analysis(adata, test_res): #非零表达基因分析
    metric2fct = {
           'pearson': pearsonr,
           'mse': mse
    }
...
def deeper_analysis(adata, test_res, de_column_prefix = 'rank_genes_groups_cov', most_variable_genes = None): #深度扰动效应分析
    ...
    # calculate mean expression for each condition  计算各条件的平均表达
    unique_conditions = adata.obs.condition.unique()
    conditions2index = {}
    for i in unique_conditions:
        conditions2index[i] = np.where(adata.obs.condition == i)[0]
    ...
    for pert in np.unique(test_res['pert_cat']):
        ...
        #方向一致性分析 
        for val in [20, 50, 100, 200]:  #在全基因组和不同DE基因子集上分别计算
            #计算预测变化方向(上调/下调)与真实变化方向的一致性比例
        ...
        if len(nonzero_des) == 0:
            ...
            #检查预测值是否落在真实值的[min, max]范围内
            in_range = (pred_mean[nonzero_des] >= min_[nonzero_des]) & (pred_mean[nonzero_des] <= max_[nonzero_des])
            frac_in_range = sum(in_range)/len(nonzero_des) #还计算了在25-75%、40-60%、45-55%分位数区间内的比例
    ...
    return pert_metric #返回各种扰动效应评估指标
