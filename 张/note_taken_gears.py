...
#GEARS (Gene Expression Analysis and Regression System) 是一个基于图神经网络(GNN)的单细胞基因扰动响应预测系统，主要用于预测基因扰动后的转录组变化和分析基因相互作用。
...
class GEARS: 
    def __init__(self, pert_data, 
                 ...
                 exp_name = 'GEARS',
                 pred_scalar = False,
                 gi_predict = False): #初始化模型环境
        
        self.weight_bias_track = weight_bias_track
        
        if self.weight_bias_track: #权重跟踪配置(如Weights & Biases)
            import wandb
        ...
        self.device = device #设备设置(CPU/GPU)
        self.config = None #参数？ 后面直接加载数据里面的
        ...
        random.seed(self.seed) #随机种子设置(确保可重复性)
    ...
    def model_initialize(self, hidden_size = 64, #配置模型超参数并初始化：
                         num_go_gnn_layers = 1, 
        ...
        if self.config['G_coexpress'] is None:
            ## calculating co expression similarity graph 构建两种基因关系图：共表达图
        ...
        if self.config['G_go'] is None:
            print('No G_go')
            ## calculating gene ontology similarity graph 构建两种基因关系图：GO图
        ...
        self.model = GEARS_Model(self.config).to(self.device) #初始化GEARS_Model核心模型
    ...
    def train(self, epochs = 20, result_dir='./results', #方法实现完整训练过程：
              lr = 1e-3,
              weight_decay = 5e-4
             ):
        ...
        self.model = self.model.to(self.device) #把初始化的模型拿出来
        ...
        optimizer = optim.Adam(self.model.parameters(), lr=lr, weight_decay = weight_decay)
        scheduler = StepLR(optimizer, step_size=1, gamma=0.5) #学习率调度器
        ...
        for epoch in range(epochs):
            ...
            #学习率调度
            scheduler.step()
            # Evaluate model performance on train and val set 验证模型
